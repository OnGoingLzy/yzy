<template>
	<view>
		<view style="background: white;width: 94vw;padding: 3vw 3vw 0 3vw;">
			...
			<view>
				<text style="font-weight: bold;">{{departmentInfo+' '+doctor}}</text>
			</view>
			<view>
				<text style="color: gray;">{{appointmentDate + ' ' +schedule.appointmentTimeSlot}}</text>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad: function (option) {
			...
			this.schedule = JSON.parse(decodeURIComponent(option.schedule));
			this.departmentInfo = JSON.parse(decodeURIComponent(option.departmentInfo));
		...
		methods: {
			...
			onInput() {
			  ...
			  if (numericValue.length > 11) {
				...
				// 否则，保留纯数字
				this.patientPhone = numericValue;
				if (/^1[3456789]\d{9}$/.test(numericValue)) {
				  // 检查是否为中国手机号
				  this.validInput = true;
				}
			  }
			...
			async submit(){
				...
				var data1 = {
					...
					appointmentDoctorScheduleId: this.schedule.id,
