<template>
	<view>
			...
			<uni-grid :column="4" :highlight="true" class="my-grid" :showBorder="false">
				...
				<uni-grid-item>
					<view class="grid-item-box" @click="toMyRecipe">
						<uni-badge :text="'new'" absolute="rightTop" type="success">
				...
				<uni-grid-item>
					<view class="grid-item-box" @click="toMyHistoryDemand">
						<image src="../../static/icon/history_demand.png" class="icon-class"></image>
						</uni-badge>
						<text class="text">历史预订</text> 
					</view>
			...
			<uni-grid :column="4" :highlight="true" class="my-grid" :showBorder="false">
				<uni-grid-item>
					<view class="grid-item-box" @click="toAddress()">
						<view style="display: flex;align-items: center;justify-content: center;" class="icon-class">
							<image src="../../static/icon/location.svg" style="width: 8vw;height: 8vw;"></image>
						</view>
				...
				<uni-grid-item>
					<view class="grid-item-box" @click="toDrugUsers()">
						...
						<image src="../../static/icon/patient2.svg" class="icon-class"></image>
				...
				<uni-grid-item>
					<view class="grid-item-box" @click="toUserAppointmentQuery">
						<image src="../../static/icon/预约.svg" class="icon-class" >			
		...
		<uni-popup ref="popup2" type="center">
			<view style="padding: 15px;border-radius: 10px;background: white;">
			      <image style="height: 70vw;width: 70vw;" :src="promotionCodeUrl" class="promotionCodeImage" :show-menu-by-longpress="true"></image>
			</view>
		</uni-popup>
	...
	export default {
		...
		methods: {
			...
			toMyHistoryDemand(){
				...
				uni.navigateTo({
					url: '/pages/goods_order_history/goods_order_history'
				})
			},
			delayNavigate() {
				this.seconds = 3
			...
			showTips(n) {
				uni.showToast({
					icon: 'none',
					title: '请授权后再之重新打开个人中心！' + n + '秒之后自动跳转到授权页',
					mask: true,
					duration: 1500
				})
			},
			  ...
			  if(!uni.getStorageSync("phone"))
			   ...
			   if(this.promotionCodeUrl=='data:image/jpeg;base64,'){
					...
					try {
					  ...
					  console.log(res)
					  if (res.code != -99) {
					    that.promotionCodeUrl = 'data:image/jpeg;base64,'+res.data;   
