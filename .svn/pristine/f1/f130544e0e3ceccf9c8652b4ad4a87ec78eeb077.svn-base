	...
	<view>
		<view class="example">
			<uni-forms label-width="100" label-position="left" label-align="left">
				...
				<!-- <uni-forms-item label="用药人手机号" required>
					<uni-easyinput placeholder="请输入真实的用药人手机号" v-model="authObj.phone" />
				</uni-forms-item> -->
	...
	export default {
		...
		methods: {
			async addGoodsUser(){
				...
				// if(!phoneRegex.test(this.authObj.phone)){
				// 	uni.showToast({
				// 		title:"请输入正确手机号",
				// 		icon:"none"
				// 	})
				...
				try {
					...
					else if (response.code == 99 &&response.data.flag==-99){
						uni.showToast({
