		...
		<view style="width: 96vw;padding:2vw">
			<view v-for="(item,i) in hospitalList" :key="i" class="hospital-card">
				<view style="	display: flex;">
					<view class="left-box">
						<image :src="item.hospitalImgId? 'http://images.yndzyf.com/getimage.ashx?mlszh='+item.hospitalImgId:defaultPic" style="width: 20vw;height: 20vw;border-radius: 10px;" mode="aspectFit"></image>
					</view>
				...
				<view class="line" style="height: 1upx;background-color: #ccc;margin: 5px 0px;"></view>
	...
	export default {
		...
		methods: {
			...
			toAppointment(hospital){
				...
				var that = this
				uni.navigateTo({
					url:"/subPackage_appointment/Choose_department/Choose_department?hospital="+encodeURIComponent(JSON.stringify(hospital))+'&date='+encodeURIComponent(JSON.stringify(that.selectedDate))
				})
