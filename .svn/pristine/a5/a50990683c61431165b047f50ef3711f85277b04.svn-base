<template>
  <view class="container">
	  ...
	  <div class="person-card-container">
		...
		<view v-if="goodsUser==null || goodsUser.length==0" style="    text-align: center;width: 100%;margin-top: 40px;">
			<text style="color: gray;">根据监管部门要求,需进行用药人实名认证方可正常购药!</text>
		</view>
	  </div>
...
export default {
  ...
  data() {
    return {
		...
		isEnableClick: true,
		choiceObj: null,
		backToConsultationDirectly: null
    }
  },
  mounted() {
	this.getGoodsUser()
  ...
  onLoad(options) {
  	if (options.openAddUser == 'true') {
  	    this.showAddFrame();
		this.backToConsultationDirectly = true;
  	}
  },
  onUnload() {
  ...
  methods: {
		...
		addOver(res){
			...
			if (this.backToConsultationDirectly === true){
				uni.navigateBack();
			}
		},
		async getGoodsUser(){
			...
			try {
				...
				if(response.code == 99){
					...
					//把本人排在前面
					if(this.goodsUser != null){
						this.goodsUser.sort((a, b) => {
