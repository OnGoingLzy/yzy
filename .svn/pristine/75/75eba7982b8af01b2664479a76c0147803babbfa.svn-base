		...
		<view >
			<scroll-view :scroll-top='leftScrollTop' scroll-y="true" class="sort-left">
				...
				<view class="sort-first" :class="{activesort : item.id == choiceFirst}" @click="sortFun(index,item.id)">
					...
					<!-- <image :src="'http://images.yndzyf.com/getimage.ashx?mlszh=21088516'" class="icon-class"></image> -->
		...
		<view class="sort-right">
			<view style="display: flex;width: 100%;justify-content: center;">
				<view class="first-category-box">
					<text v-text="firstData[index].categoryName" style="color: #fff; text-shadow: 4px 4px 4px rgba(0, 0, 0, 0.15);"></text>
				</view>
			</view>
				...
				<view class="sort-flex">
					<block v-for="(sonItem,indexs) in item.sonCategory" :key="indexs">
						<view class="sort-goods" @click="toSearch(sonItem.id.toString())">
							...
							<image :src="sonItem.categoryImg==null?defaultImg:'http://images.yndzyf.com/getimage.ashx?mlszh='+sonItem.categoryImg" mode="aspectFit" style="width: 100%;"></image>
...
export default{
	data() {
		return {
			leftScrollTop:700,//左侧滚动条位置
			choiceFirst:0, //选择了哪个一级分类
			index:0,
			firstData:[],
			secondData:[],
			defaultImg:'/static/icon/shop.svg',
			//defaultImg:'/static/糖尿病-二甲双胍.jpg',
			icon:[
