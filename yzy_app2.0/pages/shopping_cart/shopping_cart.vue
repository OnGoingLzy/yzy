<template>
	<view class="box">
		...
		<view class="box_content">
			<view class="box_content_all" v-for="items in shops" :key='items.id'>
				...
				<view class="shop-title ">
					<view class="box_content_oput_input2">
						<checkbox-group @change="shopCheckboxChange($event, items.id)">
							<checkbox :value="items.id" :checked="items.isChecked" style="padding-left: 16rpx;"/>
						</checkbox-group>
					</view>	
					<image src="/static/icon/药店.svg" style="width: 18px;height: 18px;"></image>
					<text class="shop-name"  @click="goToShop(items)">{{items.simplifyShopName}}</text>
				</view>
				...
				<view class="box_content_one" v-for="item in items.drugs" :key='item.id'>
					<view class="box_content_oput">
						...
						<view class="box_content_oput_imges" style="position: relative;">
							<image
								v-if="item.recipeStatus != 'AUDIT' && item.recipeStatus !=null"
							...
							<view v-if="item.recipeStatus != 'AUDIT' && item.recipeStatus !=null" style="    position: absolute;
																	top: 50%;
																	text-align: center;
																	transform: translate(-50%, -50%);
																	left: 50%;">
								<text style="font-size: 9px;">处方药,处方审核完成后可查看详细信息</text>
							</view>	
						</view>
					</view>
					<view class="box_content_number">
						<view class="trashDel">
							...
							<uni-icons type="trash" size="20" @click="clean(item)"></uni-icons>
						</view>
						<view class="badge">
						...
						<view class="box_content_effect">
							{{item.specification}}/{{item.sellUnit}}
						</view>
						<view class="box_content_effect_manufacturer">
							{{item.manufacturer}}
						</view>
						<view class="box_content_and">
							<view class="box_content_number_discount" v-text="'￥' + item.price"></view>
		...
		<view v-if="shops===null" style="    text-align: center;">
		...
		<view class="tabbar">
			...
			<view class="all" @click="cleanSelected">
				<text>删除所选</text>
			</view>
			<view class="totalPrice">
				<text class="totalPrice_ff">合计:</text>
				<text class="totalPrice_red" v-text="'￥' + Number(totalPrice).toFixed(2)"></text>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			//按照添加购物车的顺序显示购物车中的信息
			//排序规则：
			//1.最外层对象中的drugs数组中的每个对象，将id值按照降序排列，即大的在前，小的在后；
			//2.从最外层对象中的drugs数组中，取出drugs数组中的所有对象的最大id，以该id作为最外层对象的排序规则，以该规则对最外层对象进行降序排序，即大的在前，小的在后；
			sortShops() {
				//对每个shop的drugs数组进行排序
				this.shops.forEach(shop => {
					shop.drugs.sort((a, b) => b.id - a.id);
				});
			
				//根据每个shop的drugs数组中的最大id，对shops数组进行排序
				this.shops.sort((a, b) => {
					let maxIdA = a.drugs.length > 0 ? a.drugs[0].id : 0;
					let maxIdB = b.drugs.length > 0 ? b.drugs[0].id : 0;
			...
			setCart() { //计算总价
				...
				this.shops.forEach(shop => {
					shop.drugs.forEach(drug => {
						if (drug.isChecked) {
							totalPrice += drug.num * drug.price;
			...
			shopCheckboxChange: function(e, shopId) {
				// 找到被修改的商品对象
				const shop = this.shops.find((shop) => shop.id === shopId);
				this.shops.find((shop) => shop.id === shopId).isChecked = !this.shops.find((shop) => shop.id === shopId).isChecked
				let isChecked = this.shops.find((shop) => shop.id === shopId).isChecked
				if (shop) {
				  for (const item of shop.drugs) {
				    this.checkboxChange2(shopId, item.id,isChecked);
				  }
				}
			...
			checkboxChange: function(e, shopId, drugId) {
				...
				//判断门店是否全选
				temp2 = this.shops[shopIndex].drugs.every(drug => drug.isChecked);
				this.shops[shopIndex].isChecked = temp2

				this.setCart();
			},
			checkboxChange2(shopId, drugId,isChecked) {
				var temp = [];
				var temp2 = [];
				// 找到被修改的商品对象
				let shopIndex = this.shops.findIndex(v => v.id === shopId);
				...
				// 选中状态取反
				this.shops[shopIndex].drugs[drugIndex].isChecked = isChecked
			...
			cleanSelected(){
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							that.selectedGoods.forEach(item=>{
								that.delUserCartGoods(item);
							})
						} else if (res.cancel) {
							return
						}
